...
export function ScriptEdit() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([s, cats]) => {
        ...
        setCategoryId(s.category?.id ?? null);
        setStatus(s.status);
        setCategories(cats);
      })
      .catch((err) => {
        setFetchError(err?.status === 404 ? "Script not found." : "Failed to load script.");
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!validate() || !script) return;
    setSaving(true);
    try {
      await scriptApi.updateScript(script.id, {
        title: title.trim(),
  ...
  if (fetchError) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="rounded-lg border border-red-800 bg-red-950/30 p-4 text-sm text-red-400">{fetchError}</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <h1 className="text-foreground mb-8">Edit Script</h1>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="category" className="block text-sm text-foreground mb-1.5">Category</label>
          ...
          <div>
            <label htmlFor="status" className="block text-sm text-foreground mb-1.5">Status</label>
            <div className="relative">
              <select
                id="status"
                value={status}
        ...
        <div>
          ...
          <ScriptEditor value={scriptText} onChange={setScriptText} minRows={14} />
          {errors.scriptText && <p className="mt-1.5 text-xs text-red-400">{errors.scriptText}</p>}
        </div>
